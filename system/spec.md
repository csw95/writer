...
- 正文发布区叙述句完成一句话描述并以中文句号 `。` 收束时必须另起一段；人物发言引号内的句号除外，且该规则不约束摘要、state、run 日志、归档、表格、元数据或补充说明
...
正文发布区还必须执行句号分段：叙述句完成一句话描述并以中文句号 `。` 收束时，应另起一段；人物发言引号内的句号不触发强制分段。该规则只检查 `## 正文` 到正文分隔线 `---` 之间，不检查元数据、摘要、state、run 日志、归档、表格或 `## 名词与剧情说明`。
