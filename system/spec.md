...
- 所有章节必须基于 state 状态生成
- 每章写作前必须读取当前时间线、地点、战力资源、关键物品、敌对势力行动、伏笔状态和最近章节摘要
- 每章写作后必须更新章节摘要、场景因果链、时间地点、战力资源、关键物品、人物关系、敌对势力行动和质量风险
- `state/current-state.md` 默认只保留最近 20 章章节摘要和最近 10 章场景因果链，旧章节明细必须归档到 `state/archive/`
- 单章运行优先读取 active context；完整 cast / canon / items 只在 active 文件不足时定向读取
...
- 每章必须区分正文发布语域和系统接力语域：正文区写给读者，元数据、场景记录、Fact/Item/Loop 记录和补充说明写给接力系统
...
```
STATE 0: 初始化 → 创建 bible / world / characters / canon / items / state / open_loops
STATE 1: 结构生成 → 按篇幅档位生成 long_term_arc / part / volume / arc
STATE 2: 章节规划 → 逐章生成 chapter_plan
STATE 3: 章节生成 → AI写正文
STATE 4: 审阅修正 → 检查逻辑 / 爽点 / 连贯性
STATE 5: 状态更新 → 更新 state / canon / items / open_loops
STATE 6: 循环推进 → 进入下一章节
```
...
4. 主角战力、资源、道具、关键物品持有/位置/状态、伤势、情绪、声望和内在弧光状态。
5. 核心配角状态、关系线状态、命运节点、禁止行为边界。
6. 敌对势力目标、当前行动、资源、误判点、底牌和下一步压力。
7. 未回收伏笔、逾期风险、关联 Fact / Item ID、计划回收和最晚回收章节。
8. 世界规则、能力代价、资源守恒、关键物品生命周期、禁用设定和当前可显露/可操作信息上限。
...
4. 主角与关键人物的战力、资源、关键物品、伤势、情绪、声望、关系和知情状态。
5. 敌对势力行动结果、损失、误判、下一步计划和威胁等级。
6. 伏笔新增 / 推进 / 回收 / 废弃状态，计划回收和最晚回收章节。
7. 事实显露状态、关键物品状态、读者已知、角色知情变化和禁止改写项。
...
- 每章：连续性、场景因果、人物一致性、关键物品生命周期、设定边界、事实显露和伏笔状态。
- 每 3-5 章：局势变化、爽点轮换、情绪曲线、关系线越界风险。
- 每 10-20 章：剧情单元闭环、压力阶梯升级、敌对势力主动性、伏笔推进。
- 每卷结束：主线目标、人物命运、关系线、事实库、关键物品、世界规则、资源战力和未解决问题总检。
...
5. **items（关键物品）**: 关键物品画像、持有者、位置、公开认知、真实功能、使用条件、代价限制、损坏/消耗/转移规则、生命周期、关联 Fact/Loop 和当前 Arc 活跃物品索引 items-active
6. **story_structure（故事结构）**: 长期主线（按 length_tier）、Part（ultra_long 必填）、冲突升级路线、节奏审稿节点、主要人物长期命运线、关系线主次规划、关键物品生命周期计划、Volume（按篇幅档位）、Arc（按篇幅档位）、章节目标
7. **state（核心运行状态）**: 当前剧情阶段、运行控制字段、章节摘要活跃窗口、时间线、地点、主角状态（战力/情绪/资源/关键物品）、敌对势力状态、事实显露状态、关键物品状态、未解决冲突、质量风险、错误修复台账、归档索引、下一章目标
8. **open_loops（伏笔系统）**: 未解决剧情线索、关联事实/物品、计划回收章节、最晚回收章节、当前状态
9. **runs（运行日志）**: 单章运行、断点恢复、阶段审稿、State 归档和 Arc/Volume/Part 过渡记录，质量风险和下一次运行路由
...
章节文件中 `## 正文` 到正文分隔线 `---` 之间属于发布正文，必须像正式连载文本一样可直接给读者阅读。该区域禁止出现内部接力标记、工程标记或系统术语，包括但不限于：

```text
Ch19 / Ch21 / Chapter 20 / Scene 1 / FACT-001 / ITEM-001 / LOOP-001 / REL-MAIN-001
STATE 2 / STATE_6 / chapter_plan / canon_fact_plan / key_item_plan / pending_action / run_lock
Writer AI / Reviewer AI / State Manager AI / novel_id
```

Planner 和 State 可以在计划、状态、元数据、审阅报告和补充说明中使用这些标记；Writer 写正文时必须把它们转译为故事内表达。例如：

```text
Ch19 活证链磨出的痕 -> 方才那道活证链磨出的痕
Ch21 救人代价 -> 接下来救人要付出的代价
FACT-010 推进 -> 旧朱痕把“未审先红”的边界压实
ITEM-003 状态变化 -> 那枚旧印被谁握住、裂纹是否扩大
```

Reviewer 和机器校验必须只扫描正文发布语域；若正文区出现内部标记，本章不得进入 State Manager，必须先小修或重写对应句段。
...
- 关键物品操作：引用的 Item ID、持有者/位置/状态、允许操作、代价限制、生命周期变化、关联 Fact/Loop、禁止越界
...
- **State Manager AI**: 更新 state、characters、canon、items、open_loops、power progression
...
- 当前小说已补齐 canon/items.md，且当前 Volume / Arc 涉及的关键物品画像、生命周期、使用代价、持有位置和回收/退场窗口已登记。
- 当前小说已补齐 canon/items-active.md，且覆盖当前 Arc 和后续 3-5 章涉及 Item。
- 当前小说已补齐关系线主次规划，且当前 Volume / Arc 的关系线编排和章节级节拍已完成。
- state 当前章节与最新章节一致。
- state 的最近章节摘要、时间线、地点、人物位置、战力资源、关键物品和敌对势力行动已更新到最新章节。
- canon/facts.md 的显露状态、读者已知和角色知情变化与最新章节一致。
- canon/items.md 的持有者、位置、状态、生命周期和状态变更记录与最新章节一致。
- 当前 Arc 后续至少3章目标清晰。
- 当前 Arc 后续至少3章涉及的 Fact ID、显露层级、误导/维持节点和角色知情变化清晰。
- 当前 Arc 后续至少3章涉及的 Item ID、持有/位置/状态变化、生命周期节点、代价限制和回收/退场窗口清晰。
...
- 临场添加关键物品、改写关键物品功能/代价/持有位置/生命周期，或让未登记物品承担核心转折
- 跳过章节计划
- 写无事件推进段落
- 写没有因果变化和后续影响的场景
- 修改历史章节
...
- 用巧合、道具突然出现、关键物品临时新功能、设定补丁、反派降智或主角突然开窍解决核心矛盾
- 跳过时间线、地点、战力资源、关键物品、敌对势力和章节摘要状态更新
...
- 在 chapter_plan 未声明 Item ID 和 key_item_plan 的情况下使用关键物品推进核心转折、证据链、伏笔回收或危机解决
...
- Planner / Writer 优先使用 `cast-active.md`、`facts-active.md`、`items-active.md` 和 `current-state.md`；只有 active 文件无法覆盖本章需求时，才定向读取完整 `cast.md`、`facts.md` 或 `items.md`。
- 每次阶段过渡、关键人物变化、Fact 窗口变化或关键物品生命周期变化后，必须刷新 active context，避免长期全量读取导致上下文预算被设定文件吞没。
