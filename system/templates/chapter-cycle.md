...
- `novels/{novel_id}/canon/items.md` 已完成，且当前 Volume / Arc 涉及的关键物品画像、持有位置、真实功能、代价限制、生命周期和回收/退场窗口已登记
- `novels/{novel_id}/canon/items-active.md` 已完成，且覆盖当前 Arc 和后续 3-5 章涉及 Item
...
- 当前 Arc 的章节级关键物品持有、位置、状态、生命周期、代价限制和回收/退场节点已明确
- 当前 state 文件已更新到最新章节
...
- 当前 state 文件的最近章节摘要、时间线、地点、人物位置、战力资源、关键物品、敌对势力行动、质量风险和错误修复台账已更新到最新章节
- 当前 state 文件未超过归档阈值；若超过，先执行 `pending_action: state_archive`
- 当前 canon/facts 文件已更新到最新章节
- 当前 canon/items 文件已更新到最新章节
...
- `novels/{novel_id}/canon/items-active.md`（优先读取）
...
- 当前 state 中的时间线、地点、人物位置、战力资源、关键物品、敌对势力行动和最近章节摘要
...
- [ ] key_item_plan 已声明本章引用的 Item ID、当前持有者/位置/状态、允许操作、代价限制、生命周期变化和关联 Fact/Loop
- [ ] 本章涉及的关键物品均已在 canon/items.md 中登记，且被 items-active.md 覆盖或已刷新 active context
- [ ] 本章关键物品操作不超过当前 Volume / Arc 的生命周期计划和操作上限
- [ ] pre_write_state_check 已确认时间线、地点、人物位置、战力资源、关键物品、敌对势力行动和上一章钩子
- [ ] time_location_continuity 已声明本章故事时间、地点变化、移动成本和人物可达性
- [ ] 每个关键场景包含前因、目标、阻碍、行动、选择、结果变化和后续影响
- [ ] 每个关键场景包含呈现焦点、至少2个感官锚点、动作节拍、句式节奏和禁止概括项
- [ ] power_resource_plan 已声明战力、能力、资源、道具、关键物品、伤势或地位变化；无变化也写明“无”
...
- canon/items（只读）
- items-active（只读，优先于完整 canon/items）
- 当前 Volume / Arc（只读）
- 当前 Volume / Arc 的关系线编排（只读）
- 当前 Volume / Arc 的事实显露计划（只读）
- 当前 Volume / Arc 的关键物品生命周期计划（只读）
- 当前时间线、地点、人物位置、战力资源、关键物品、敌对势力行动和最近章节摘要（只读）
...
- [ ] `## 正文` 到 `---` 的正文发布区不包含 Ch/Chapter 编号、FACT/ITEM/LOOP/REL ID、Scene/STATE、chapter_plan/state 字段名、AI 角色名或运行控制字段等内部标记
- [ ] 章节末尾包含 `## 名词与剧情说明`，且该区块不计入正文字数
- [ ] 名词与剧情说明只解释本章已显露信息，没有新增正文未写事件、未登记事实或超出显露窗口的剧透
- [ ] 按 scenes 顺序完成主要场景
- [ ] 每个关键场景都产生局势、人物、关系、伏笔、情绪、设定显露、战力资源、关键物品或地点状态中的至少一项变化
...
- [ ] 无未登记关键物品、未规划物品操作、关键物品状态越界或违背 canon/items.md 的内容
- [ ] 关键物品使用、显露、损坏、转移或退场符合 chapter_plan.key_item_plan
- [ ] 人物说话、行动和选择符合 cast.md 中的画像、底线和阶段命运方向
- [ ] 时间线、地点、人物位置、战力资源、关键物品和敌对势力行动与 state 连续
- [ ] 战力、能力、资源、道具、关键物品和伤势变化符合 world/items 约束和 chapter_plan.power_resource_plan / key_item_plan
...
- 当前 Volume / Arc 的事实显露计划
- 当前 Volume / Arc 的关键物品生命周期计划
- 当前时间线、地点、战力资源、关键物品、敌对势力行动、最近章节摘要和错误修复台账
...
- 存在未登记关键物品、违背 canon/items.md、关键物品持有/位置/状态不连续、功能/代价/生命周期越界、或未规划物品解决核心矛盾 → 停止正文流程，回到关键物品台账和章节规划补齐
- 存在时间线矛盾、地点移动不成立、战力资源不守恒、人物行为无触发条件、反派降智、核心矛盾靠巧合或临场补丁解决 → 阻塞，回到规划或内容补齐
- 存在无因果变化场景，且无法合并为有效推进 → 返回 STATE 3 重写
- 正文发布区出现少量内部标记且可不改剧情局部替换 → 进入 STATE 4b 小修；多处泄漏或导致正文依赖系统上下文 → 返回 STATE 3 重写对应场景
...
由 Writer AI 只按 review 指定范围修改正文，不得新增 chapter_plan 未声明的事件、人物、关系推进、事实显露、关键物品操作或战力资源变化。
...
- 更新 `novels/{novel_id}/canon/items.md` 中本章涉及关键物品的持有者、位置、状态、生命周期、关联章节计划和状态变更记录（不得临场改写真实功能、代价限制和禁止改写项）
- 更新 `novels/{novel_id}/canon/items-active.md` 中当前 Arc 活跃关键物品、即将触发物品和暂不触碰物品
- 更新 state / cast 中已推进的关系线状态、触发事件、信任度或关系强度变化、下一计划节点
- 更新 `novels/{novel_id}/open-loops/loops.md`
- 更新章节摘要、场景因果链、时间线、地点、人物位置、战力资源、关键物品、敌对势力行动、质量风险和错误修复台账
...
- [ ] canon/items.md 已同步本章关键物品持有者、位置、状态、生命周期和关联章节计划
- [ ] 未改写 canon/items.md 的真实功能、代价限制和禁止改写项
- [ ] open_loops 中本章推进/新埋/回收的伏笔已记录
- [ ] 最近章节摘要已追加本章目标、局势变化和下一章承接点
- [ ] 时间线、地点、人物位置、战力资源、关键物品、伤势、声望和道具变化已同步
...
- [ ] STATE 5: state、canon、items 和 open_loops 已更新
- [ ] STATE 5: 章节摘要、场景因果链、时间线、地点、战力资源、关键物品、敌对势力行动和错误修复台账已更新
